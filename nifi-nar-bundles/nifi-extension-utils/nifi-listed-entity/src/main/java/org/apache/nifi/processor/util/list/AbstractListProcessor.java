...
public abstract class AbstractListProcessor<T extends ListableEntity> extends AbstractProcessor implements VerifiableProcessor {
    ...
    public static final PropertyDescriptor DISTRIBUTED_CACHE_SERVICE = new Builder()
            ...
            .build();
    public static final PropertyDescriptor DISTRIBUTED_HBASE_SERVICE = new Builder()
            .name("Distributed Cache Service")
            .description("Save file name to hbase service")
            .required(true)
    ...
    static final PropertyDescriptor COLUMNS = new PropertyDescriptor.Builder()
            .name("Columns")
            .description("<colFamily>:<colQualifier>")
            .required(true)
            .build();
    ...
    public void writeFileNameToHbase(String fileName, ProcessContext context) throws IOException {
        HBaseClientService hBaseClientService = context.getProperty(DISTRIBUTED_HBASE_SERVICE).asControllerService(HBaseClientService.class);
        String tableName = context.getProperty(TABLE_NAME).getValue();
        String[] familyQualifier = context.getProperty(COLUMNS).getValue().split(":");

        final Collection<PutColumn> columns = Collections.singletonList(new PutColumn(familyQualifier[0].getBytes(StandardCharsets.UTF_8),
                familyQualifier[1].getBytes(StandardCharsets.UTF_8),
                fileName.getBytes(StandardCharsets.UTF_8)));
        hBaseClientService.put(tableName, null, columns);
    }
    ...
    private int createFlowFilesForEntities(final ProcessContext context, final ProcessSession session, final Map<Long, List<T>> orderedEntries) {
        ...
        for (final Map.Entry<Long, List<T>> timestampEntities : orderedEntries.entrySet()) {
            ...
            for (final T entity : entities) {
                ...
                FlowFile flowFile = session.create();
                flowFile = session.putAllAttributes(flowFile, attributes);
                String fileName = flowFile.getAttribute("absolute.path") + flowFile.getAttribute("filename");
                try {
                    writeFileNameToHbase(fileName, context);
                } catch (Exception e) {
